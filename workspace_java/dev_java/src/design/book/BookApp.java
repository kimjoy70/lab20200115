...
//ActionListener에 정의된 추상메소드를 재정의할것.	
		...
		if(jbtn_ins==obj) {
			...
			if(indexs.length==0) {
				...
				for(int i=0;i<indexs.length;i++) {
					pMap = new HashMap<>();
					int choice = indexs[0];
					int b_no = Integer.parseInt(dtm_book.getValueAt(choice, 0).toString());
					pMap.put("b_no", b_no);
					JOptionPane.showMessageDialog(this, "선택한 로우 "+b_no);
				}
			}
			//JOptionPane.showMessageDialog(this, "선택한 로우2 "+pMap);
			bList = bDao.getBookList(pMap);
			//JOptionPane.showMessageDialog(this, "bList.size():"+bList.size());
			if(bList.size()>0) {
				rMap = bList.get(0);
			}
	...
	public void refreshData(Map<String,Object> pMap) {
		System.out.println("refreshData 호출 성공");
		List<Map<String,Object>> bList = bDao.getBookList(pMap);
		//JOptionPane.showMessageDialog(this, bList.get(0).get("b_img"));
		...
		for(int i=0;i<bList.size();i++) {
			...
			v.add(rMap.get("b_no"));
			v.add(rMap.get("b_name"));
			v.add(rMap.get("author"));
			v.add(rMap.get("publish"));
			dtm_book.addRow(v);
		}
	}
