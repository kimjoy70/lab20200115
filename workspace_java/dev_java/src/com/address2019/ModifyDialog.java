...
//컴넌트들의 위치를 정해줍니다.
	...
	// 확인버튼 선택시 작업을 정의합니다.
  	private void btnOkayActionPerformed(ActionEvent evt) {
  		if(getName().trim().length() == 0) {
  			optionDlg.showMessageDialog(this, "이름을 입력하세요.", "Error",
		 ...
		 */
			...
			try {
				...
				vo.setBirthday(getBirthDay());
				vo.setComments(getComment());
				vo.setRegistedate(Util.getCurrentDate());

				// Controller에 데이터 입력을 의뢰하고 전체 데이터를 다시 조회합니다.
				AddressCtrl ctrl= new AddressCtrl(vo);
				AddressVO newVo= ctrl.send(vo);
				abook.refreshData();
			} catch (Exception e) {
				optionDlg.showMessageDialog(this, "입력중 에러가 발생했습니다." + e,
	...
	public String getGender() {
		if (comboGender.getSelectedItem().
				equals("남자")) return "1";
		else return "2";
	}
	...
	public Vector getUserValue() {
		Vector retValue = null;
		if (!isCancel) {
			retValue = new Vector();

			retValue.addElement(txtName.getText());
			retValue.addElement(txtAddress.getText());
			retValue.addElement(txtTel.getText());
		}
