...
public class SKTelecom extends JFrame implements ActionListener{
	...
	JButton jbtn_withdraw = new JButton("출금");
	JButton jbtn_exit = new JButton("종료");
	JPanel jp_north = new JPanel();
	JPanel jp_north1 = new JPanel();
	JPanel jp_north2 = new JPanel();
	JLabel jlb_wnum = new JLabel("내계좌");
	JTextField jtf_wnum = new JTextField(12);
	...
	public void setAccountBank(String nickName) {
		...
		for(int i=0;i<baList.size();i++) {
			...
			if(nickName.equals(rMap.get("mem_id").toString())) {
				jtf_wnum.setText(rMap.get("accountnumber").toString());
				jtf_bala.setText(rMap.get("balance").toString());
			}
	...
	public void connect_process() {
		...
		try {
			...
			SKTelecomThread cbt = new SKTelecomThread(this);
